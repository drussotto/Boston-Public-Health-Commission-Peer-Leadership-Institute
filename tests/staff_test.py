...
class StaffTest(PliEntireDbTestCase):
    ...
    def add_dummy_staff(self, client, active):
        with open(os.path.join(os.path.dirname(__file__), "res", "python-logo.png"), "r") as picture:
            res = post_add_staff(client, "Dummy", "Test Title", "some bio", picture, "fakeemail@fakeemail.com", "111-111-1111", active)
    ...
    def test_deactivate_staff(self, client):
        ...
        res = post_edit_staff(client, {"_id": ex.staff1["_id"], "active": False})
        self.assertEqual(200, res.status_code)
        staff_page = client.get('/page/staff.html')
    ...
    def test_activate_staff(self, client):
        ...
        res = post_edit_staff(client, {"_id": ex.staff1["_id"], "active": True})
        self.assertEqual(200, res.status_code)
        staff_page = client.get('/page/staff.html')
        self.assertTrue(ex.staff_inactive["name"] in staff_page.data)
        self.assertTrue(ex.staff_inactive["active"])

    @with_login(user1["email_address"], user1["real_pass"])
    def test_edit_staff(self, client):
        res = post_edit_staff(client, {"_id": ex.staff1["_id"], "name": "Dummy", "Title": "Dummy Title"})
        self.assertEqual(200, res.status_code)
        staff_page = client.get('/page/staff.html')
        self.assertEqual(ex.staff1["name"], "Dummy")
        self.assertEqual(ex.staff1["title"], "Dummy Title")
        self.assertFalse("Dummy" in staff_page.data)
    ...
    def test_edit_staff_unauthorized(self, client):
        res = post_edit_staff(client, {"_id": ex.staff1["_id"], "name": "Dummy"})
        self.assertEqual(403, res.status_code)
